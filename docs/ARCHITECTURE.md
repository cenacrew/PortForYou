...
graph TB
    ...
    subgraph "Orchestration"
        ...
        SchedHealth["Cloud Scheduler: pfy-health-checks (*/30 min)"]
...
graph TB
    ...
    Manual["Déclenchement manuel uniquement"] --> Sonar["sonarcloud.yml\nquality gate + couverture lcov (api, web, template-back-core, template-front-core)"]
...
- **`ci.yml`** : pipeline unique validation → déploiement. Auth CI via **Workload Identity Federation** (aucune clé JSON). Le déploiement est sérialisé (`concurrency: deploy-platform, cancel-in-progress: false`) pour ne jamais interrompre une release en cours. Les jobs `templates-back`/`templates-front` sont conditionnés au job de filtre `changes` (`dorny/paths-filter`, pattern `needs`+`if`) : ils ne tournent que si `templates/**`, `packages/template-back-core/**` ou `packages/template-front-core/**` a changé — sinon ils passent en statut `skipped`, traité comme réussi par les required checks de branch protection. Le job `deploy` scanne les images `api`/`web` avec Trivy (`CRITICAL,HIGH`, `exit-code: 1`) juste après le build/push et avant le `gcloud run deploy` correspondant, puis termine par un smoke test (`curl --fail` avec retries) sur `/api/v1/health` de l'API et la home de la vitrine.
  L'URL Cloud Run de `pfy-api` est récupérée **avant** tout build/déploiement (`gcloud run services describe`, une révision Cloud Run ne change jamais d'URL) plutôt qu'après : sert à la fois au build-arg `NEXT_PUBLIC_API_URL` du front et aux env vars `WEB_ORIGIN`/`API_PUBLIC_URL` du déploiement de l'API elle-même, dans le **même** `gcloud run deploy`. Avant ce choix, un second appel `gcloud run services update pfy-api` recréait une révision Cloud Run supplémentaire à chaque déploiement pour réappliquer des valeurs de toute façon déjà identiques — doublant inutilement le nombre de révisions (et le coût associé) de `pfy-api` à chaque merge sur `main`.
...
- **`sonarcloud.yml`** : quality gate (bugs probables, code smells, duplication, dette technique), déclenché **uniquement manuellement** (`workflow_dispatch`) — pas sur push `main` ni sur PR : sur le plan gratuit de SonarCloud le quality gate n'est pas personnalisable (couverture/duplication du code nouveau non désactivables), donc toute analyse automatique poste un check en échec (croix rouge) non pertinent et non actionnable ; on rafraîchit le dashboard à la demande plutôt que de polluer main/les PR. Génère la couverture Vitest (`test:coverage`, lcov ; `test:coverage:emu` pour `apps/api` — sans l'émulateur Firestore les tests d'intégration s'auto-skippent et la couverture mesurée est artificiellement basse) de `apps/api`, `apps/web`, `packages/template-back-core` et `packages/template-front-core`, puis la passe à Sonar via `sonar-project.properties` (`sonar.javascript.lcov.reportPaths`). Nécessite la création du projet sur sonarcloud.io et le secret `SONAR_TOKEN` (action manuelle du mainteneur) — sans lui, l'étape d'analyse est ignorée (le check reste vert).
...
| ---------------------------------------- | ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Health checks tenants                    | `pfy-health-checks` (Cloud Scheduler, */30 min)      | `POST /internal/health-checks` → ping `/` et `/api/v1/health` de chaque site `live`, met à jour `sites/{id}.health`                                                                                                                                                                                                                                                                                |
...
| Uptime checks                            | toutes les 15 min, 3 régions                         | `pfy-api` + 3 tenants démo, `/api/v1/health` — détecte un service down sans trafic (angle mort des alertes 5xx). `infra/scripts/setup-uptime-checks.sh`, alerte sur le même canal email. Période au maximum autorisé (15 min) et régions réduites au minimum (3) : chaque ping réveille un service scale-to-zero et facture un cold start — réglage retenu après l'incident de coûts du 2026-07-23 |
