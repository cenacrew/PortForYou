...
function parseStartAfter(raw: unknown): string | null {
...
router.get('/artworks', async (req, res) => {
  try {
    ...
    const startAfter = parseStartAfter(req.query.startAfter);
    ...
    const nextCursor = snapshot.docs.length === limit ? (snapshot.docs.at(-1)?.id ?? null) : null;
